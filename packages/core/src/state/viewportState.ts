...
import type { Viewport, ViewportState } from '../engine/chartTypes'
import type { VisibleRange } from '../engine/layout/pane'
import { getVisibleRange } from '../engine/viewport/viewport'
...
export function createViewportState(deps: ViewportDeps) {
  ...
  const { signals, readonly } = createSubState(
    ...
    },
    {
      ...
      dpr: (s) =>
        computeDpr(s.viewWidth(), s.viewHeight(), s.preciseDpr()),
      /** Physical plot width. */
      ...
      scrollLeftLogical: (s) =>
        s.scrollLeft() - computeLeftLoadBufferWidth(s.viewWidth()),
      /** Visible range — derived from viewport + options + dataLength. */
      visibleRange: (s) => {
        const vp = computeViewport(
          s.viewWidth(), s.viewHeight(), s.scrollLeft(),
          computeLeftLoadBufferWidth(s.viewWidth()),
          s.preciseDpr(),
        )
        const opts = deps.options$()
        return getVisibleRange(
          vp.scrollLeft, vp.plotWidth, opts.kWidth, opts.kGap,
          deps.dataLength$(), vp.dpr,
        )
      },
      ...
      viewportState: (s) => {
        ...
        const vp = computeViewport(
          s.viewWidth(), s.viewHeight(), s.scrollLeft(), llbw, s.preciseDpr(),
        )
        const opts = deps.options$()
